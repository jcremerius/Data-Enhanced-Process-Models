 ...
 * 
 * Adapted from: github - Swing-range-slider
 */
class RangeSliderUI extends SlickerSliderUI {
	
	/** Color of selected range. */
	private Color rangeColor = new Color(150, 150, 150);
	
	private boolean drawUpper = false;
	...
	protected double getSliderPercentage() {
		if (!drawUpper) {
			double value = this.slider.getValue() - this.slider.getMinimum();
			double max = this.slider.getMaximum() - this.slider.getMinimum();
			return value / max;
		} else {
			double value = ((RangeSlider) this.slider).getUpperValue() - this.slider.getMinimum();
			double max = this.slider.getMaximum() - this.slider.getMinimum();
			return value / max;
		}
	}
