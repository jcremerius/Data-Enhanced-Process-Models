...
public class AlignedLogVisualisationDataImplWaiting extends AlignedLogVisualisationDataImplSojourn {
	...
	public AlignedLogVisualisationDataImplWaiting(IvMModel model, PerformanceWrapper performance, IvMLogInfo logInfo) {
	...
	protected void computeExtremes(PerformanceWrapper performance) {
		...
		for (long d : performance.getNodeMeasures(TypeNode.waiting, Gather.average)) {
			if (d >= 0 && d > maxMeasure) {
				maxMeasure = d;
			}
			if (d >= 0 && d < minMeasure) {
	...
	public Triple<String, Long, Long> getNodeLabel(int unode, boolean includeModelMoves) {
		long length = performance.getNodeMeasure(TypeNode.waiting, Gather.average, unode);
		if (length > -1) {
